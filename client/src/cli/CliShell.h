...
// server responses and to recognize a login_result reply so `login` can
// block until it knows whether it actually succeeded, instead of firing
// the request and leaving the caller to guess. Once run() returns
// successfully, client/src/main.cpp installs GameClient as the next
// message handler for the game phase -- same deferred-handoff pattern
// WebSocketClient::setOnMessage was built for.
class CliShell
{
public:
    struct LoginOutcome
